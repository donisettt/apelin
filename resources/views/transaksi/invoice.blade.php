...
<head>
    ...
    <link rel="icon" href="/bootstrap/assets/img/lgo.png">
    <title>Invoice</title>
    <style>
        body {
    ...
    <div class="invoice">
        ...
        <table>
            @foreach ($items as $item)
                <tr>
                    <td>
                        {{ $item->qty }} {{ $item->nama_paket }} x {{ number_format($item->harga, 0, ',', '.') }}
                        <br>
                        <small>Ket : {{ $item->keterangan }}</small>
                    </td>
                    <td class="right">{{ number_format($item->harga * $item->qty, 0, ',', '.') }}</td>
                </tr>
            @endforeach
        </table>
        <hr>
